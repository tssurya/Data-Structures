...
//creating a template class for heap.
 ...
 {
    private:
        vector<type> heap; //the underlying container used to store the Max-heap.
        size_t heap_size; //contains the size of the heap.

 	public:
        // to maintain the Max-heap property by propagating the node at the 'position' upwards.
        void up_heapify(int position);

        // to maintain the Max-heap property by prapagating the node at the 'position' downwards.
        ...
        Max_Heap(typename vector<type>::iterator heap_begin, typename vector<type>::iterator heap_end);

        // constructor to create a heap for the whole vector.
        Max_Heap(vector<type> heap);
        ...
        // to return the Maximum element of the heap i.e the topmost element.
        type top()
        {
            return heap[1];
        }

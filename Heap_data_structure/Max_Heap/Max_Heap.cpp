...
template<class type>
Max_Heap<type>:: Max_Heap(typename vector<type>:: iterator data_begin,typename vector<type>:: iterator data_end)
{
...
template<class type>
Max_Heap<type>:: Max_Heap(vector<type> data)
{
...
3 else swap the parent with the largest of its children.
4 return to step 1. */

template<class type>
void Max_Heap<type> :: down_heapify(int index)
{
    int left_child = index << 1;
    int right_child = left_child | 1;
    int largest=index;

    if(left_child < heap.size() and heap[left_child]<heap[largest])
    {
        largest=left_child;
    }

    if(right_child < heap.size() and heap[right_child]<heap[largest])
    {
        largest=right_child;
    }
...
template<class type>
void Max_Heap<type>::up_heapify(int index)
{
...
template<class type>
void Max_Heap<type>:: push_into_heap(type value)
{
...
template<class type>
void Max_Heap<type>::pop_from_heap()
{
    if(heap.size()==1)
    {
        heap.clear();
...
Since the Max-heap contains the largest element at the top always ,
...
template<class type>
void Max_Heap<type>::heap_sort(typename vector<type>:: iterator start,typename vector<type>:: iterator finish)
{
    typename vector<type>:: iterator it=start;
    Max_Heap<type>  temp_heap(start,finish);//creating a heap using the constructor.
